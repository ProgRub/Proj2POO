...
public class FestivalCinema {
    ...
    private void criarPerito() {
        ...
        for (Perito peritoExistente : edicoes.get(indexEdicoes).getPeritos()) { //verifica se o perito criado já existe
            if (perito.equals(peritoExistente)) {
                System.out.println("Esse perito já existe.");
                return;
            }
        }
        edicoes.get(indexEdicoes).inserePerito(perito); //insere o perito na lista de peritos da edição, caso ele não exista
    }
    ...
    private void escolherAtoresPrincipaisCandidatos(Premio p, boolean homem) {
        if (p.getAtoresCandidatos().size() != 4) { //se ainda não foram escolhidos os candidatos
            ...
            int i = 1;
            int contaCandidatosPossiveis = 0;
            if (homem) { //se for o prémio de melhor ator principal
                for (Filme filme : edicoes.get(indexEdicoes).getFilmes()) {
            ...
            } else { //se for o prémio de melhor atriz principal
                for (Filme filme : edicoes.get(indexEdicoes).getFilmes()) {
